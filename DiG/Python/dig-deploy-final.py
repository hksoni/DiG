...
alevin_java_jar = './alevin-ext.jar'
...
def main(physical_net, virtual_net, mapping):
    global phy_g, virt_g, pos_phy, pos_virt, root, node_mappings, canvas_frame, canvas, plt_fig, \
        continue_button, connect_vm_button
    root = Tk.Tk()
    ...
    Tk.Label(root, text="Virtual Network", fg="dark green").grid(row=0, column=1)
    ...
    connect_vm_button = Tk.Button(root, text="Connect to VM",fg="dark green", command = on_connect_vm_button)
    connect_vm_button.grid(row=2, column=1)
    ...
    connect_vm_button.config(state='disabled')
    Tk.mainloop()
...
def create_menu():
    ...
    filemenu.add_command(label="Open Virtual Network", command=get_experimental_net)
    filemenu.add_separator()
    filemenu.add_command(label="Exit", command=root.quit)
    menubar.add_cascade(label="Select Files", menu=filemenu)
    root.config(menu=menubar)
    mapping_menu = Tk.Menu(menubar, tearoff=0)
    mapping_menu.add_command(label="RUN ALEVIN", command=generate_mapping)
...
def generate_mapping():
    global physical_net_f, virtual_net_f, mapping_f, node_mappings
    if physical_net_f == None or virtual_net_f == None:
        return
    mapping_f = os.getcwd() + str('/mapping.txt')
    mapping_cmd = 'java -jar ' + alevin_java_jar + ' ' + physical_net_f + ' ' + virtual_net_f + ' ' + mapping_f
    commands = [mapping_cmd]
    [error_msg, ret_code] = execute_ordered_list_cmds(commands)
    if ret_code != 0:
        error_msg = 'could not create mapping'
        messagebox.showerror(title="Mapping", message=error_msg)
        return
...
def close_virtual_host(vhost_w):
    global  vhost_windows
    vhost_w.quit()
    vhost_w.destroy()
    vhost_windows.remove(vhost_w)


def reintialize():
    global root, plt_fig, phy_g, virt_g, pos_virt, pos_phy, phy_subplot, virt_subplot, phy_sp_xlim, \
        phy_sp_ylim, virt_sp_xlim, virt_sp_ylim, canvas, node_mappings, continue_button, \
        virt_color_map, host_color_map, deployment_host_color_map, poller, \
        physical_net_f, virtual_net_f, mapping_f, canvas_frame
    root = None
    plt_fig = None
    phy_g = None
    virt_g = None

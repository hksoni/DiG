...
class GroupExecution:
    ...
    def run_group(self):
        ...
        virtual_host_executor_sender.execute(self.group_ip, self.sender_command,
                                             self.group_ip+"_s_"+get_str_time())
        ...
        for vh, ia, rd in self.mem_ia_rd_l:
            ...
            virtual_host_executor_receiver.execute(self.group_ip, self.receiver_command,
                                                   self.group_ip+"_r_"+get_str_time())
            Logger.LogReceiver()
...
def main(physical_net_f, virtual_net_f, node_mappings_file, bw_map_file, exe_schedule, sender_schedule, num_groups):
    ...
    Logger.OpenFile("./receivers_count.log")
    ...
    if sender_schedule is None:
        ...
        # group_sender_inter_arrival = np.random.randint(1,2, no_of_groups)
    else:
    ...
    if exe_schedule is None:
        ...
        json.dump(group_sender_inter_arrival.tolist(), open("group_sender_sched.txt",'w'))
        print("-------------------------------------------------------------")
    ...
    for l in range(0,no_of_groups):
        ...
        group_start_log.writelines(str(time.strftime("%I:%M:%S +000", time.localtime()) + "-"+ip+"\n").decode('unicode-escape'))
    ...
    for t in group_terminate_threads_list:
        t.join()
    Logger.CloseFile("./receivers_count.log")
